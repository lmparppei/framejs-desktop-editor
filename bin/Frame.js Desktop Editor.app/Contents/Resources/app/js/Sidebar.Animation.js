...
/*
		var row = new UI.Row();
		row.add(new UI.Text('Color').setWidth ( '90px' ) );
		container.add( row );

		var color = new UI.Input( animation.color );
		color.onChange( function () {
			animation.color = this.getValue();
			//signals.effectColor.dispatch( animation.ecolor );

			tools.recolor(editor.selected, this.getValue());
		});
		row.add( color );
